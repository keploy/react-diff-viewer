...
function formatStr(str) {
    var replaceCurlOpen = str.replace(/{/gi, "{\n");
    var replaceCurlClose = replaceCurlOpen.replace(/}/gi, "\n}");
    var replaceSqBrackOpen = replaceCurlClose.replace(new RegExp('[', 'gi'), "[\n");
    var replaceSqBrackClose = replaceSqBrackOpen.replace(new RegExp(']', 'gi'), "\n]");
    var replaceComma = replaceSqBrackClose.replace(/,/gi, ",\n");
    return replaceComma;
    // let i = 0;
...
var computeLineInformation = function (oldString, newString, noise, disableWordDiff, compareMethod, linesOffset) {
    ...
    var diffArray = diff.diffLines(formatStr(expected.trimRight()), formatStr(actual.trimRight()), {
        newlineIsToken: true,

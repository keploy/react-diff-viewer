...
const computeLineInformation = (oldString, newString, noise, disableWordDiff = false, compareMethod = DiffMethod.CHARS, linesOffset = 0) => {
    ...
    const getLineInformation = (value, diffIndex, added, removed, evaluateOnlyFirstLine, LineIndexTobeReturned) => {
            ...
            if (added || removed) {
                ...
                if (removed) {
                    ...
                    if (nextDiff && nextDiff.added) {
                        ...
                        // console.log()
                        if (nextDiffLines) {
                            const { value: rightValue, lineNumber, type, } = getLineInformation(nextDiff.value, diffIndex, true, false)[lineIndex].right;

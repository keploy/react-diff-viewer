...
function noiseDiffArray(count, expectedObj, actualObj, key) {
    var result = [];
...
function CompareJSON(expectedStr, actualStr, noise, flattenKeyPath) {
    var result = [];
    var expectedJSON = JSON.parse(expectedStr), actualJSON = JSON.parse(actualStr);
    ...
    if (typeof expectedJSON !== typeof actualJSON) {
        if (!noise.includes(flattenKeyPath)) {
            ...
            var output = noiseDiffArray(-2, expectedJSON, actualJSON, "");
            ...
            // both are objects and not null
                ...
                for (var key in expectedJSON) {
                    ...
                    if (key in actualJSON) {
                        var valueExpectedObj = expectedJSON[key], valueActualObj = actualJSON[key];

...
function CompareJSON(expected, actual, noise, flattenKeyPath) {
    var result = [];
    ...
    switch (typeof expectedValue) {
        ...
        case "object": {
            if (noise.includes(flattenKeyPath)) {
                var linesExpected = constructLines(JSON.stringify(expectedValue, null, 2));
                var linesActual_1 = constructLines(JSON.stringify(actualValue, null, 2));
                linesExpected.map(function (el, elIndex) {
                    if (elIndex < linesActual_1.length) {
                        result.push({ count: -2, value: el + "_keploy_|_keploy_" + linesActual_1[elIndex] });
                    }
                    else {
                        result.push({ count: -2, value: el + "_keploy_|_keploy_ " });
                    }
                });
                for (var indx = linesExpected.length; indx < linesActual_1.length; indx++) {
                    result.push({ count: -2, value: " _keploy_|_keploy_" + linesActual_1[indx] });
                }
                return result;
            }
            ...
            if (Array.isArray(expectedValue) && Array.isArray(actualValue)) {
                ...
                expectedValue.map(function (el, elIndx) {
                    if (elIndx < actualValue.length) {
                        var output = CompareJSON(JSON.stringify(el, null, 2), JSON.stringify(actualValue[elIndx], null, 2), noise, flattenKeyPath);
                    ...
                    // add extra elements of expected as of type removed
                        ...
                        var lines_1 = constructLines(JSON.stringify(el, null, 2));
                        lines_1.map(function (line, lineIndex) {
                            line = "  " + line;
                            if (lineIndex == lines_1.length - 1) {
                                line = line + ",";
                            }
                ...
                var _loop_1 = function (indx) {
                    var lines = constructLines(JSON.stringify(actualValue[indx], null, 2));
                            ...
                            if (valueActualObj == null && valueExpectedObj == null) {
                                ...
                                if (output.length === 1) {
                                    if (output[0].count === -1) {
                                        ...
                                        var tagStartIndex = output[0].value.indexOf("_keploy_|_keploy_"), tagLength = "_keploy_|_keploy_".length;
        ...
        return lines
            ...
            if (added || removed) {
                ...
                if (diffArray[diffIndex].count === -1) {
                    ...
                    var tagStartIndex = value.indexOf("_keploy_|_keploy_"), tagLength = "_keploy_|_keploy_".length;
